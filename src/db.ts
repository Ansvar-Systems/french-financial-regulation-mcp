/**
 * SQLite database access layer for the AMF/ACPR French Financial Regulation MCP server.
 *
 * Schema mirrors the AMF/ACPR regulatory structure:
 *   - sourcebooks        — top-level regulatory sourcebook directories (AMF_Reglement_General, AMF_Positions, etc.)
 *   - provisions         — individual rules, positions, recommandations, and instructions
 *   - enforcement_actions — AMF/ACPR sanctions and enforcement decisions
 *
 * FTS5 virtual tables back full-text search on provisions and enforcement actions.
 * Language: French (provisions stored in French).
 */
...
export function searchProvisions(opts: SearchProvisionsOptions): Provision[] {
  ...
  if (opts.sourcebook ?? opts.status) {
    ...
    if (opts.sourcebook) {
      ...
      params["sourcebook"] = opts.sourcebook;
    }
    if (opts.status) {
...
export function getProvision(
  ...
  const row = db
    ...
    .get(sourcebook, reference) as Provision | undefined;

  if (row) return row;

  // Fuzzy fallback: partial reference match
  return (
    (db
      .prepare(
        "SELECT * FROM provisions WHERE sourcebook_id = ? AND reference LIKE ? LIMIT 1",
      )
      .get(sourcebook, `${reference}%`) as Provision | undefined) ?? null
  );
}
...
export function getDataFreshness(): DataFreshness {
  const db = getDb();
  const provisions_count = (
    db.prepare("SELECT COUNT(*) as count FROM provisions").get() as { count: number }
  ).count;
  const enforcement_count = (
    db.prepare("SELECT COUNT(*) as count FROM enforcement_actions").get() as { count: number }
  ).count;
  const latest = db
    .prepare("SELECT MAX(date) as latest FROM enforcement_actions")
    .get() as { latest: string | null };
  return {
    provisions_count,
    enforcement_count,
    latest_enforcement_date: latest.latest,
  };
}
